...
const dpaUpdates: NewsItem[] = [
  ...
  },
  {
    ...
    date: '2026-09-24',
    heat: 8,
    title: 'ICO执法成果落地：TikTok撤回两项上诉、接受1270万英镑儿童数据罚款终局，上级裁判所“艺术目的”先例后推荐系统调查恢复推进',
    summary:
      '2026年9月24日，ICO宣布TikTok已同意支付2023年开出的1270万英镑罚款并撤回上诉（罚单终局），同时撤回对推荐系统信息通知书的上诉，使ICO自2025年2月开启的13-17岁用户推荐系统数据处理调查得以继续推进。2023年罚单认定：TikTok未有效核验和清退13岁以下儿童账号（据经济分析估算2020年最多175万英国低龄儿童使用）、未就儿童数据收集使用与共享提供清晰信息、未取得家长同意。副专员Emily Keaney表示“我们始终清楚该处理不属于特殊目的”，并欢迎TikTok在上级裁判所2026年8月驳回“艺术目的”抗辩后撤回上诉；ICO估算其推动儿童数据改进的工作已惠及近500万儿童用户。此为ICO于9月30日转型信息委员会前的标志性执法收尾。',
    overallImpact:
      '英国儿童数据执法三年拉锯终局，“艺术目的”豁免抗辩被司法封堵，ICO执法威慑获得背书；推荐系统调查恢复推进后，后续认定若成立将适用DUAA下提高的罚款上限（1750万英镑或全球营业额4%），风险量级远高于本次罚单。',
    industryImpact:
      '中国出海企业启示：1) 出海短视频/社交产品的推荐算法对未成年人的画像与分发逻辑须留存完整合规文档（年龄核验、默认隐私、内容分层）；2) 监管已具备量化违规规模的“经济分析”工具，违规成本测算应纳入产品决策；3) 英国ICO与欧盟DPA在儿童数据上执法趋同，建议按最严法域统一配置儿童保护基线。',
    tags: ['ICO', 'TikTok', '英国', '儿童数据', '罚款', '推荐系统', '执法', '出海合规'],
  ...
  },
  {
    ...
    date: '2026-09-17',
    heat: 7,
    title: '欧盟法院佐审官Canal+案意见：向ISP“合作伙伴”作出的概括性营销同意无效，数据主体须知悉控制者身份否则须重新取得同意——背书CNIL对Canal+的60万欧元罚款',
    summary:
      '2026年9月17日，欧盟法院佐审官Spielmann就C-317/25 Groupe Canal+案（法国最高行政法院请求初步裁决）发表意见：数据收集时向ISP“合作伙伴”作出的概括性营销同意，仅在自由、具体、知情且明确时有效，“知情”要求数据主体能知悉控制者身份；若同意时控制者身份未知，须在营销前（最迟首次通讯时）重新取得同意，首次通讯后的退订选项不能治愈同意瑕疵。即便法院认为类别授权足够，该类别也须精确到数据主体可合理预期被联系的程度，而“合作伙伴”过于宽泛。该意见实质背书法国CNIL对Canal+处以60万欧元罚款的执法逻辑（涉案营销覆盖约390万人）。佐审官意见不约束法院，但法院多数遵循其结论，终审判决预计未来数月作出。',
    overallImpact:
      'GDPR同意规则在第三方定向营销场景的关键澄清：ISP/平台以“合作伙伴”打包获取营销同意的欧洲常见广告获客模式将被系统性否定，广告主与数据中介必须建立“逐控制者、逐目的”的同意链。与EDPB“consent or pay”指南、DMA数据结合限制同向叠加，第三方数据驱动的精准广告合规复杂度显著上升。',
    industryImpact:
      '中国出海企业启示：1) 欧盟营销同意须列明具体接收方或可识别的类别边界，身份确定后重新请求同意；2) 经ISP、运营商、应用商店等渠道间接获客时，须核验上游同意链完整性与粒度，渠道方“已获同意”的保证不足以免责；3) 广告SDK/数据共享合同应加入同意链条陈述与保证条款，分配举证责任；4) 跟踪CJEU终审判决作为欧盟定向广告同意架构的最终依据。',
    tags: ['CJEU', 'GDPR', '同意', '定向营销', 'CNIL', 'Canal+', '数据中介', '广告合规', '出海合规'],
